...
class Problem_001_BestSolution {
...
class Problem_001_StudySolution {
    public int[] solution(int[] lottos, int[] win_nums) {
        int zero = 0;		// 0 갯수 카운트
        int matched = 0;	// 일치할 경우 카운트
        ...
        int[] answer = {Math.min(7 - max, 6), Math.min(7 - min, 6)};	// Math.min(A , B) : A와 B중 작은 수로 결정
        return answer;
    }
}