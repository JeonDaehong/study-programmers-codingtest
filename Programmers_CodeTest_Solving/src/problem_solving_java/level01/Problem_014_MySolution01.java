...
//풀이시간    : 2021-11-08 19:45 ~ 19:59
//결과	  	  : 100.0 / 100.0 (정확성: 100.0)
//나의 판단   : 잘 풀었는데, '/'을 '%'로 실수하는등의 잔실수 안하기. 꼼꼼하게 풀기.
//			    HashSet을 이용하면, 아래의 종류 골라내는 코드를 굳이 만들지 않아도 됨.
...
* <입.출력 예시>
	...
	★ 입출력 예 #3
	 	→ 6마리의 폰켓몬이 있으므로, 3마리의 폰켓몬을 골라야 합니다.
		  가장 많은 종류의 폰켓몬을 고르기 위해서는
		  3번 폰켓몬 한 마리와 2번 폰켓몬 두 마리를 고르거나,
		  혹은 3번 폰켓몬 두 마리와 2번 폰켓몬 한 마리를 고르면 됩니다.
		  따라서 최대 고를 수 있는 폰켓몬 종류의 수는 2입니다.

*/

public class Problem_014_MySolution01 {

	public int solution(int[] nums) {
		
		List<Integer> list = new ArrayList<>(); // 종류
        int answer = 0;
        int cnt = 0;
        ...
        } else if ((nums.length / 2 ) > list.size()) {
        	answer = list.size();
        } else if ((nums.length / 2) == list.size()) {
        	answer = list.size();
        }
