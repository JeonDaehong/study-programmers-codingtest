...
* <입.출력 예시>
	...
	★ 입출력 예 #1
		→ 각 부서에서 [1원, 3원, 2원, 5원, 4원]만큼의 금액을 신청했습니다.
		  만약에, 1원, 2원, 4원을 신청한 부서의 물품을 구매해주면
...
public class Problem_018_MySolution01 {

	public int solution(int[] d, int budget) {
		
        int answer = 0;
        
        Arrays.sort(d);		// 필요 예산이 작은 부서부터 오름차순으로 정리
        
        // 남은 예산이 d배열의 수보다 작아질때까지 예산 분배
        for (int i : d) {
        	if (budget >= i) {
        		budget -= i;
        		answer ++;
